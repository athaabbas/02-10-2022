...
<div class="container-fluid mt-3">
  ...
  <form action="proses_ubah_siswa.php" method="post">
    ...
    "<?=$data_siswa['nama_siswa']?>" class="form-control">

    Tanggal Lahir :
    <input type="date" name="tanggal_lahir" value=
    "<?=$data_siswa['tanggal_lahir']?>" class="form-control">
    ...
    <select name="gender" class="form-control">
      ...
      ?>
      <option value="<?=$key_gender?>" <?=$select?>> 
      <?=$val_gender?> </option>
      <?php endforeach ?>
    </select>
    ...
    "<?=$data_siswa['username']?>" class="form-control">
